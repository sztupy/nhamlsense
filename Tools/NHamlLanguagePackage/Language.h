...
public:
	...
	BEGIN_COM_MAP(Language)
		COM_INTERFACE_ENTRY(INHamlLanguage)
	...
	STDMETHODIMP GetSource(IVsTextBuffer* pBuffer, INHamlSource** ppSource);

	/********** IVsLanguageInfo **********/
    STDMETHODIMP GetLanguageName( 
        /* [out] */ __RPC__deref_out_opt BSTR *bstrName)
	{
		*bstrName = SysAllocString(L"NHaml");
		return S_OK;
	}
	...
	{
		*pbstrExtensions = SysAllocString(L".haml");
		return S_OK;
	}
