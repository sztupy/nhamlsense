...
{
	*pbstrProductDetails = CComBSTR(VERSIONINFO_PRODUCT " integration package for Visual Studio 2008. For more information see http://nhamlviewengine.com.\r\n" VERSIONINFO_COPYRIGHT).Detach();
	return S_OK;
}
...
STDMETHODIMP Package::SetSite(IServiceProvider* site)
{
	...
	_HR(proffer->ProfferService(__uuidof(NHamlLanguageService), this, &_dwProfferCookie));

	// Create an appdomain based out of the location of this com dll. 
	// Managed NHaml.dll and NHamlLanguage.dll assemblies are in the same location
	...
	_HR(domainSetup->put_ApplicationName(CComBSTR(L"NHaml Language Package Domain")));

	CComPtr<IUnknown> punkDomain;
	_HR(pRuntime->CreateDomainEx(L"NHaml Language", punkSetup, NULL, &punkDomain));
	...
	_HR(appDomain->CreateInstance(CComBSTR(L"NHamlLanguage"), CComBSTR(L"NHamlLanguage.LanguageSupervisor"), &supervisorHandle));
...
{
	if (guidService == __uuidof(NHamlLanguageService))
	{
		//NHamlLanguage::Language^ lang = gcnew NHamlLanguage::Language();
