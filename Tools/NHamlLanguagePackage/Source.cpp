...
STDMETHODIMP Source::GetDefaultPageBaseType(BSTR* pPageBaseType)
{
	...
	while(SUCCEEDED(hr))
	{
		...
		if (CompareStringW(LOCALE_INVARIANT, NORM_IGNORECASE, name, -1, L"NHaml.Web.Mvc", -1) == CSTR_EQUAL)
		{
			pageBaseType = L"NHaml.Web.Mvc.NHamlView";
			break;
		}
		else if (CompareStringW(LOCALE_INVARIANT, NORM_IGNORECASE, name, -1, L"Castle.MonoRail.Views.NHaml", -1) == CSTR_EQUAL)
		{
			pageBaseType = L"Castle.MonoRail.Views.NHaml.NHamlView";
			break;
		}
	}
